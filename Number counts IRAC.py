...
input_file = 'C:/Users/olive/Dropbox/5. semester/Special kursus/flux_values_IRAC.txt'
...
output_file_y_values = 'ns_IRAC.txt'
...
# Mask zero and NaN values in y_value_interpolated
masked_y_values = np.ma.masked_where(np.isnan(y_value_interpolated) | (y_value_interpolated == 0), y_value_interpolated)

# Find the minimum non-zero value along each column
nlim = np.min(masked_y_values, axis=0)

# Replace the minimum non-zero values with the corresponding zero values
nlim.data[nlim.mask] = 0
...
input_file_radius = 'C:/Users/olive/Dropbox/5. semester/Special kursus/angular_distances_IRAC.txt'
...
output_file = 'additional_calculations_IRAC.txt'
